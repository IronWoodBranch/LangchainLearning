...
- pydantic的使用
  1. 类要继承BaseModel
  2. 枚举类型，类的用法，主要是为了更好的输出和规范，感觉像是工程上的抉择，最好在用的时候补上
  3. 具体作用，等于把json的结果提前规定好，只是一种手段/技术，用的时候细看，虽然重要但不新奇
  4. 可选字段：会生成none和0的区别，语法查文档熟练，optional
  5. 默认值：先作为知识储备进行理解，可能在一些默认配置的层面，可以进行操作，但直觉上感觉这类似的配置应该使用更强硬的限制手段，而非各个平台要求不同的默认值
  6. 枚举：和别的语言一样，标准化限制，用法查文档
